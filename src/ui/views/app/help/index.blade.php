...
<head>
    ...
    <title>{{ $title or 'Help' }}</title>
    <script id="btccom" type="text/plain">
        {!! json_encode([
            'lang' => \App::getLocale()
        ]) !!}
    </script>
    <script id="en" type="text/plain" src="./en.md?__inline"></script>
    <script id="zh-cn" type="text/plain" src="./zh-cn.md?__inline"></script>
...
<script>
    window.__btccom = JSON.parse(document.querySelector('#btccom').textContent);

    const marked = require('modules/lib/marked');
    const markdown = document.querySelector(`#${window.__btccom.lang}`).textContent;

    document.querySelector('#wrapper').innerHTML = marked(markdown);
</script>
